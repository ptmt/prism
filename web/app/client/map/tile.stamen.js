...
[
  ...
  function deprecate(base, flavors) {
    ...
    for (var i = 0; i < flavors.length; i++) {
      ...
      PROVIDERS[flavor] = MAKE_PROVIDER(flavor, provider.type, provider.minZoom,
        provider.maxZoom);
  ...
  function setupFlavors(base, flavors, type) {
    ...
    for (var i = 0; i < flavors.length; i++) {
      var flavor = [base, flavors[i]].join("-");
      PROVIDERS[flavor] = MAKE_PROVIDER(flavor, type || provider.type,
   ...
   */
  exports.extendMM = function (MM) {
    if (typeof MM === "object") {
      var ModestTemplate = (typeof MM.Template === "function") ? MM.Template :
      ...
      MM.StamenTileLayer.prototype = {
        setCoordLimits: function (map) {
          ...
          if (provider.extent) {
            map.coordLimits = [
                      map.locationCoordinate(provider.extent[0]).zoomTo(
                provider.minZoom),
                      map.locationCoordinate(provider.extent[1]).zoomTo(
                provider.maxZoom)
                  ];
            return true;
          } else {
  ...
  if (typeof OpenLayers === "object") {
    ...
    OpenLayers.Layer.Stamen = OpenLayers.Class(OpenLayers.Layer.OSM, {
      initialize: function (name, options) {
        ...
        return OpenLayers.Layer.OSM.prototype.initialize.call(this, name,
          hosts, options);
      }
    });
  }
